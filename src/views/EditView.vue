...
async function load() {
  try {
    ...
    const data = res.data
    form.name = data.name || ''
    form.countryCodes = (data.countryCodes ? Array.from(data.countryCodes) : (data.countries || [])).join(',')
    form.startDate = data.startDate || data.start_date || ''
    form.description = data.description || ''
    form.imageIndex = data.imageIndex || 0
    form.status = data.status || 'ACTIVE'
  } catch (e) {
    console.error('Error cargando conflicto', e)
    toast.error('Error cargando conflicto')
  }
}
...
async function save() {
  ...
  try {
    const res = await api.put(`/api/v1/conflicts/${id}`, payload)
    console.log('update response', res)
    toast.success('Conflicto actualizado')
    ...
    if (err.response) {
      ...
      toast.error('Error actualizando conflicto: mira detalles abajo')
    } else {
      lastServerError.value = String(err.message || err)
      toast.error('Error actualizando conflicto (sin respuesta del servidor).')
    }
  } finally {
